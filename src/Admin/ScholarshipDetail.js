...
import { fetchCountry,
    fetchProgram,
    fetchUniversity,fetchScholarAllDetail,submitScholarshipDetail,fetchScholarship} from "../Redux/Reducers";
...
const ScholarshipDetail = () => {
  ...
  const scholarship_list = useSelector((state) => state.educationReducer.scholarship_list);
  ...
  const submitHandler =async(e)=>{
    e.preventDefault();
  await  dispatch(submitScholarshipDetail(data))
   await dispatch(fetchScholarAllDetail());

    if(state?.submit_scholarship){
      toast.success(state?.submit_scholarship,{
        position: 'top-right'
      });
    }
  ...
  useEffect(() => {
    dispatch(fetchScholarAllDetail());
  ...
  return (
    <div>
     <Toaster />
      <h4 className="text-center text-secondary my-5">Scholarship Details</h4>
      <div className="d-flex justify-content-between align-items-center mb-3">
        <Button style={{backgroundColor:"#00008b"}} variant="contained" onClick={handleOpen}>
          Add Scholarship Detail
        </Button>
      </div>
          ...
          <div className="column">
            <Box sx={{ minWidth: 120 }}>
              <FormControl fullWidth className='my-2'>
                <InputLabel id="age-label">Scholarship Name</InputLabel>
