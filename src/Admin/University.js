...
import { fetchUniversity,submitUniversity } from "../Redux/Reducers";
...
const University = () => {
  ...
  const country = useSelector((state) => state.educationReducer.university);
  ...
  const submitHandler =async(e)=>{
    e.preventDefault();
  await dispatch(submitUniversity(name))
    await dispatch(fetchUniversity());
    if(state?.error_university){
      toast.error(state?.error_university,{
        position: 'top-right'
      });
    }
    if(state?.success_university){
      toast.success('Submitted University',{
        position: 'top-right'
      });
    }
  ...
  return (
    <div>
     <Toaster />
      <h4 className="text-center text-secondary my-5">Universities</h4>
