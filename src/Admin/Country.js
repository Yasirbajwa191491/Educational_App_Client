...
import { fetchCountry,submitCountry } from "../Redux/Reducers";
...
const Country = () => {
  ...
  const submitHandler =async(e)=>{
    e.preventDefault();
  await dispatch(submitCountry(name))
    await dispatch(fetchCountry());
    if(state?.error_country){
      toast.error(state?.error_country,{
        position: 'top-right'
      });
    }
    if(state?.success_country){
      toast.success('Submitted Department',{
        position: 'top-right'
      });
    }
  ...
  return (
    <div>
     <Toaster />
      <h4 className="text-center text-secondary my-5">Countries</h4>
