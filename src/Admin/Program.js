...
import { fetchProgram,submitProgram } from "../Redux/Reducers";
...
const Program = () => {
  ...
  const country = useSelector((state) => state.educationReducer.program);
  ...
  const submitHandler =async(e)=>{
    e.preventDefault();
  await  dispatch(submitProgram(name))
   await dispatch(fetchProgram());
    if(state?.error_program){
      toast.error(state?.error_program,{
        position: 'top-right'
      });
    }
    if(state?.success_program){
      toast.success('Submitted Program',{
        position: 'top-right'
      });
    }
  ...
  return (
    <div>
     <Toaster />
      <h4 className="text-center text-secondary my-5">Programs</h4>
