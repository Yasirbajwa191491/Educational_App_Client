...
const ScholarshipResult = () => {
  ...
  return (
    <div>
      ...
      {state?.scholarship_details?.map((curEle) => {
        return (
          <div style={cardStyles} className="card my-3">
      <p style={textStyles}>
       {curEle.scholarship} for program {curEle.program}  in  
      ...
      <p style={footerStyles} className="card-footer mt-3">
        Starting Month: {curEle.startMonth} {curEle.startYear}, Duration: {curEle.duration}
      </p>
    </div>
        );
      })}
